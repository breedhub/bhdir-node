...
class Daemon extends EventEmitter {
    ...
    onListening() {
        ...
        try {
            if (this._directory.socketMode && !isNaN(this._directory.socketMode))
                fs.chmodSync(sock, this._directory.socketMode);
            if (this._directory.socketUid !== null && this._directory.socketGid !== null)
                fs.chownSync(sock, this._directory.socketUid, this._directory.socketGid);
        } catch (error) {
        ...
        try {
            ...
            switch(message.command) {
                ...
                case 'download':
                    ...
                    break;
                case 'network-create':
                    this.emit('network_create', id, message);
                    break;
                case 'network-join':
                    this.emit('network_join', id, message);
                    break;
                case 'node-create':
                    this.emit('node_create', id, message);
                    break;
                case 'node-roles':
                    this.emit('node_roles', id, message);
                    break;
                case 'index':
