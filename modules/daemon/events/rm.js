...
class Rm {
    ...
    /**
     * Service name is 'modules.daemon.events.rm'
     * @type {string}
     */
    static get provides() {
        return 'modules.daemon.events.rm';
    }
    ...
    handle(id, message) {
        ...
        this._logger.debug('del', `Got RM command`);
        let reply = (success, value) => {
            ...
            this._logger.debug('del', `Sending RM response`);
        ...
        this.directory.rm(name)
            .then(() => {
                reply(true);
            })
            .catch(error => {
                reply(false, error.message);
                this._logger.error(new WError(error, 'Rm.handle()'));
            });
    }
